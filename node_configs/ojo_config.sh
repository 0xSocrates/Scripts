...
exec > /dev/null 2>&1
curl -Ls http://snapshot.corenode.info/ojo_testnet/addrbook.json > $HOME/$DirectName/config/addrbook.json
curl -Ls http://snapshot.corenode.info/ojo_testnet/genesis.json > $HOME/$DirectName/config/genesis.json
